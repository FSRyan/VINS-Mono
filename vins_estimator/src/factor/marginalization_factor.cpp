...
void ResidualBlockInfo::Evaluate()
{
    ...
    for (int i = 0; i < static_cast<int>(block_sizes.size()); i++)
    {
        ...
        raw_jacobians[i] = jacobians[i].data();//矩阵的指针，这个技巧需要研究一下，从matrix类型转换到double类型
...
void MarginalizationInfo::addResidualBlockInfo(ResidualBlockInfo *residual_block_info)
{
    ...
    std::vector<int> parameter_block_sizes = residual_block_info->cost_function->parameter_block_sizes();       // 记载参数块长度的数组
    //定义每一个参数块的指针及其大小的map
...
//    std::cout<<"A:----------- "<<A.rows()<<std::endl;
    ...
    std::cout<<"A:----------- "<<A.rows()<<std::endl;
    //分解的方法参考下面的连接和讲解的链接
    //https://wenku.baidu.com/view/40b58727ad02de80d5d84073.html 54页
...
bool MarginalizationFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    Eigen::VectorXd dx(n);
    std::cout<<"marginalization_info->keep_block_size.size(): "<<marginalization_info->keep_block_size.size()<<std::endl;
    std::cout<<"m: "<<m<<std::endl;
    std::cout<<"n "<<n<<std::endl;
